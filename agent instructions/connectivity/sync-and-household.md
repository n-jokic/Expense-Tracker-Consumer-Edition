...
**Field schemas (`FIELD_SCHEMAS`):** each table maps field → `date|str|float|bool`; unknown fields REJECTED (never dropped). Type coercion: `date` via `date.fromisoformat(str(v)[:10])`, `str` via `str(v)`, `float` via `float(v)` + `math.isfinite` check, `bool` via explicit `("1","true","yes","on")`/`("0","false","no","off")`/`0/1` — `bool("false")==True` is NOT used. `status` enum: `active|closed` for savings_accounts. Category/subcategory validated against `CATEGORIES`/`ALL_SUBCATS` AFTER legacy remap.
